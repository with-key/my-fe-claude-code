...
기획 문서 분석부터 TDD, UI 구현, 코드 리뷰까지 프론트엔드 개발 전 과정을 자동화합니다.
...
| `/spec` | 기획 문서를 분석하여 프론트엔드 기능명세서를 생성 |
...
2. **프론트엔드 기능명세서 생성** — 분석 결과를 `docs/specs/<기능명>.md`에 정리합니다.
   - 기능 개요, 화면별 상세 명세, 상태 관리, 데이터 흐름
   - 엣지 케이스 및 예외 처리 방안
   - 기획자에게 피드백할 질문 사항과 모호한 정책 정리

**산출물:**
```
docs/specs/<기능명>.md   ← 프론트엔드 기능명세서
```
...
│                │                                        │
│                ▼                                        │
│  [Step 4]  /review  ──────────── 코드 리뷰 → 커밋        │
│                                                         │
│  ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─  │
│                                                         │
│  [기획 변경 시]                                           │
│  /impact → /spec → /tdd → /ui build → /review           │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
| 에이전트 | 역할 | 사용하는 스킬 |
|---------|------|-------------|
| `spec-analyzer` | 기획 문서 분석 및 기능명세서 생성 | `/spec` |
